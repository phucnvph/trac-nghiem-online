...
<div id="payment-modal" class="modal modal-fixed-footer" style="width: 60%; max-width: 600px;">
    <div class="modal-content">
        <h4><i class="material-icons left">qr_code</i>Thanh toán QR Code</h4>
    ...
    <div class="modal-footer">
        <a href="#!" class="modal-close waves-effect waves-red btn-flat" id="btn-cancel">Đóng</a>
        <a href="#!" id="btn-show-qr" class="waves-effect waves-blue btn blue" style="display: none;">
            <i class="material-icons left">qr_code</i>Thanh toán
        </a>
        <!-- <a href="#!" id="btn-confirm-payment" class="waves-effect waves-green btn green" style="display: none;">
            <i class="material-icons left">check</i>Đã chuyển khoản
        </a> -->
        <a href="#!" id="btn-close-success" class="waves-effect waves-green btn green modal-close" style="display: none;">
...
$(document).ready(function(){
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.payment_status === "Paid") {
                        ...
                        pay_status = 'Paid';
                        M.toast({html: 'Thanh toán thành công! Tự động phát hiện.', classes: 'green'});
                    }
                },
                error: function() {
    ...
    function generateQRCode(orderCode, amount) {
        const bankCode = 'VPBank';
        const accountNumber = '0335841799';
        const content = orderCode;
        const template = 'compact';
        
        const qrUrl = `https://qr.sepay.vn/img?bank=${bankCode}&acc=${accountNumber}&template=${template}&amount=${amount}&des=${content}`;
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.status === 1) {
                    $('#payment-success').show();
                    $('#btn-close-success').show();
                    M.toast({html: 'Thanh toán thành công!', classes: 'green'});
                } else {
                    M.toast({html: response.message, classes: 'red'});
                    $('#qr-code-section').show();
                    $('#btn-confirm-payment').show();
